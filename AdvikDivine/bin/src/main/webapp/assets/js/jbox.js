function jBox(type, options) {
        ...
        Notice: {
            ...
            _onOpen: function() {
                jQuery.each(jQuery(".jBox-Notice"), function(t, i) {
                    (i = jQuery(i)).attr("id") != this.id && i.data("jBox-Notice-position") == this.options.attributes.x + "-" + this.options.attributes.y && (this.options.stack ? i.css("margin-" + this.options.attributes.y, parseInt(i.css("margin-" + this.options.attributes.y)) + this.wrapper.outerHeight() + 10) : i.data("jBox").close({
                        ignoreDelay: !0
                    }))
        ...
        Image: {
            ...
            _onInit: function() {
                this.images = this.currentImage = {}, this.imageZIndex = 1, this.attachedElements && jQuery.each(this.attachedElements, function(t, i) {
                    if (!(i = jQuery(i)).data("jBox-image-gallery")) {
                ...
                this.showImage = function(s) {
                    if ("open" != s) {
                        ...
                        e = (e = this.currentImage.id + (1 * ("prev" == s) ? -1 : 1)) > this.images[o].length - 1 ? 0 : e < 0 ? this.images[o].length - 1 : e
                    } else {
                        o = this.source.data("jBox-image-gallery");
                        var e = this.source.data("jBox-image-id");
                    ...
                    var n = e + 1;
                    n = n > this.images[o].length - 1 ? 0 : n < 0 ? this.images[o].length - 1 : n, !jQuery("#jBox-image-" + o + "-" + n).length && jQuery('<img src="' + this.images[o][n].src + '">').load(function() {
                        t(o, n, !0)
                    })
                }
            },
    ...
    return this.IE8 = -1 != userAgent.indexOf("msie") && 8 == parseInt(userAgent.split("msie")[1]), this.prefix = -1 != userAgent.indexOf("webkit") ? "-webkit-" : "", this._getOpp = function(t) {
        ...
        if (!this.wrapper) {
            ...
            if (this.setContent(this.options.content, !0), this.setTitle(this.options.title, !0), this.options.draggable)("title" == this.options.draggable ? this.titleContainer : this.options.draggable.length > 0 ? this.options.draggable : this.options.draggable.selector ? jQuery(this.options.draggable.selector) : this.wrapper).addClass("jBox-draggable").on("mousedown", function(t) {
        ...
        var i = function(t, i) {
            if ("center" != this.options.position[i]) return this.pos[t] += t == this.options.position[i] ? -1 * this.dimensions[i] - this.options.offset[i] : this.options.offset[i], this.pos[t];
            this.pos[t] -= Math.ceil(this.dimensions[i] / 2)
        }.bind(this);
        this.wrapper.css({
            left: i("left", "x"),
            top: i("top", "y")
        ...
        if (this.options.closeOnEsc && jQuery(document).on("keyup.jBox-" + this.id, function(t) {
            ...
            var t, i = 0,
                s = function() {
                    ...
                    t || (s - i > 150 && (this.options.reposition && this.position(), this.options.adjustTracker && this._adjustPosition(), i = s), t = setTimeout(function() {
        ...
        if (!this.IE8) {
            ...
            jQuery.each(["open", "close"], function(o, e) {
                if (!this.options.animation[e] || !t[this.options.animation[e]] || "close" == e && !this.options.fade) return "";
                t[this.options.animation[e]].positions ? jQuery.each(["top", "right", "bottom", "left"], function(t, o) {
                    s += i(e, o)
                }) : s += i(e)
        ...
        var s = i.top > this.pos.top - (this.options.adjustDistance.top || 0),
            ...
            n = i.left > this.pos.left - (this.options.adjustDistance.left || 0) ? "left" : o ? "right" : null,
            a = s ? "top" : e ? "bottom" : null;
        if (n || a) {
            "move" == this.options.adjustPosition || n != this.outside && a != this.outside || ("mouse" == this.target && (this.outside = "right"), ("top" == this.outside || "left" == this.outside ? i[this._getXY(this.outside)] - (this.targetDimensions[this._getTL(this.outside)] - i[this._getTL(this.outside)]) - this.targetDimensions[this._getXY(this.outside)] + this.options.offset[this._getXY(this.outside)] : this.targetDimensions[this._getTL(this.outside)] - i[this._getTL(this.outside)] - this.options.offset[this._getXY(this.outside)]) > this.dimensions[this._getXY(this.outside)] + parseInt(this.options.adjustDistance[this._getOpp(this.outside)]) && (this.wrapper.css(this._getTL(this.outside), this.pos[this._getTL(this.outside)] + (this.dimensions[this._getXY(this.outside)] + this.options.offset[this._getXY(this.outside)] * ("top" == this.outside || "left" == this.outside ? -2 : 2) + this.targetDimensions[this._getXY(this.outside)]) * ("top" == this.outside || "left" == this.outside ? 1 : -1)), this.pointer && this.wrapper.removeClass("jBox-pointerPosition-" + this.pointer.position).addClass("jBox-pointerPosition-" + this._getOpp(this.pointer.position)).css("padding", 0).css("padding-" + this.outside, this.pointer.dimensions[this._getXY(this.outside)]), this.pointer && this.pointer.element.attr("class", "jBox-pointer jBox-pointer-" + this.outside), this.positionAdjusted = !0, this.flipped = !0));
            var h = "x" == this._getXY(this.outside) ? a : n;
            if (this.pointer && "flip" != this.options.adjustPosition && this._getXY(h) == this._getOpp(this._getXY(this.outside))) {
                if ("center" == this.pointer.align) var r = this.dimensions[this._getXY(h)] / 2 - this.pointer.dimensions[this._getOpp(this.pointer.xy)] / 2 - parseInt(this.pointer.element.css("margin-" + this.pointer.alignAttribute)) * (h != this._getTL(h) ? -1 : 1);
                else r = h == this.pointer.alignAttribute ? parseInt(this.pointer.element.css("margin-" + this.pointer.alignAttribute)) : this.dimensions[this._getXY(h)] - parseInt(this.pointer.element.css("margin-" + this.pointer.alignAttribute)) - this.pointer.dimensions[this._getXY(h)];
                spaceDiff = h == this._getTL(h) ? i[this._getTL(h)] - this.pos[this._getTL(h)] + this.options.adjustDistance[h] : -1 * (i[this._getOpp(this._getTL(h))] - this.pos[this._getTL(h)] - this.options.adjustDistance[h] - this.dimensions[this._getXY(h)]), h == this._getOpp(this._getTL(h)) && this.pos[this._getTL(h)] - spaceDiff < i[this._getTL(h)] + this.options.adjustDistance[this._getTL(h)] && (spaceDiff -= i[this._getTL(h)] + this.options.adjustDistance[this._getTL(h)] - (this.pos[this._getTL(h)] - spaceDiff)), spaceDiff = Math.min(spaceDiff, r), spaceDiff <= r && spaceDiff > 0 && (this.pointer.element.css("margin-" + this.pointer.alignAttribute, parseInt(this.pointer.element.css("margin-" + this.pointer.alignAttribute)) - spaceDiff * (h != this.pointer.alignAttribute ? -1 : 1)), this.wrapper.css(this._getTL(h), this.pos[this._getTL(h)] + spaceDiff * (h != this._getTL(h) ? -1 : 1)), this.positionAdjusted = !0)
            }
        }
...
jBox.prototype.attach = function(t, i) {
    return t || (t = jQuery(this.options.attach.selector || this.options.attach)), i || (i = this.options.trigger), t && t.length && jQuery.each(t, function(t, s) {
        (s = jQuery(s)).data("jBox-attached-" + this.id) || ("title" == this.options.getContent && void 0 != s.attr("title") && s.data("jBox-getContent", s.attr("title")).removeAttr("title"), this.attachedElements || (this.attachedElements = []), this.attachedElements.push(s[0]), s.on(i + ".jBox-attach-" + this.id, function(t) {
    ...
    return t || (t = this.attachedElements || []), t && t.length && jQuery.each(t, function(t, i) {
        (i = jQuery(i)).data("jBox-attached-" + this.id) && (i.off(i.data("jBox-attached-" + this.id) + ".jBox-attach-" + this.id), i.data("jBox-attached-" + this.id, null)), this.attachedElements = jQuery.grep(this.attachedElements, function(t) {
            return t != i[0]
        })
    }.bind(this)), this
    ...
    return null == t || void 0 == t ? this : (!this.wrapper && this._create(), this.title || (this.titleContainer = jQuery("<div/>", {
        class: "jBox-title"
    }), this.title = jQuery("<div/>").appendTo(this.titleContainer), this.wrapper.addClass("jBox-hasTitle"), ("title" == this.options.closeButton || !0 === this.options.closeButton && !this.options.overlay) && (this.wrapper.addClass("jBox-closeButton-title"), this.closeButton.appendTo(this.titleContainer)), this.titleContainer.insertBefore(this.content)), this.title.html(t), !i && this.options.repositionOnContent && (s != this.wrapper.height() || o != this.wrapper.width()) && this.position(), this)
    ...
    if (t || (t = {}), this.target = t.target || this.target || this.options.target || jQuery(window), this.dimensions = {
        ...
        var s = function(t) {
            if (-1 != jQuery.inArray(this.options.position[t], ["top", "right", "bottom", "left", "center"])) {
                var i = this.options.attributes[t] = "x" == t ? "left" : "top";
                this.pos[i] = this.targetDimensions[i], "center" != this.options.position[t] ? (i != this.options.position[t] && (this.pos[i] += this.targetDimensions[t] - this.dimensions[t]), (this.options.outside == t || "xy" == this.options.outside) && (this.pos[i] += this.dimensions[t] * (i != this.options.position[t] ? 1 : -1))) : this.pos[i] += Math.ceil((this.targetDimensions[t] - this.dimensions[t]) / 2)
            } else this.pos[this.options.attributes[t]] = this.options.position[t]
        }.bind(this);
    ...
    var i = function() {
        this.source && this.options.getTitle && this.source.attr(this.options.getTitle) && this.setTitle(this.source.attr(this.options.getTitle)), this.source && this.options.getContent && (this.source.data("jBox-getContent") ? this.setContent(this.source.data("jBox-getContent"), !0) : this.source.attr(this.options.getContent) && this.setContent(this.source.attr(this.options.getContent), !0)), this._fireEvent("onOpen"), (this.options.ajax && this.options.ajax.url && (!this.ajaxLoaded || this.options.ajax.reload) || t.ajax && t.ajax.url) && this.ajax(t.ajax || null), (!this.positionedOnOpen || this.options.repositionOnOpen) && this.position({
            target: t.target
    ...
    return e.beforeSend = function() {
        e.spinner && (this.wrapper.addClass("jBox-loading"), this.spinner = jQuery(!0 !== e.spinner ? e.spinner : '<div class="jBox-spinner"></div>').appendTo(this.container)), s.bind(this)()
