		...
		<div class="page-inner py-4">
			<div class="d-flex align-items-left align-items-md-center flex-column flex-md-row">
				...
				<div class="ml-md-auto mb-4 py-2 py-md-0">
					...
					<a href="#" class="btn btn-sm btn-border btn-blue btn-round mr-2">/ <spring:message code="site.admin.manage.user" /></a>
								...
								<tbody id="tbd">
							 	   <c:forEach items="${userList.content}" var="user" varStatus="userCount"> 
										<tr>
											...
											<button class="btn btn-warning btn-xs" onclick="editApplicationById('${user.userId}')" disabled="disabled" title="Edit User">
												<i class="fa fa-edit" ></i>
											</button>
											</c:when>
											<c:otherwise>
											<button class="btn btn-warning btn-xs" onclick="editApplicationById('${user.userId}')" title="Edit User">
												<i class="fa fa-edit" ></i>
											</button>
											</c:otherwise>
											</c:choose>
											
											<button class="btn btn-primary btn-xs" onclick="viewApplicationById('${user.userId}')" title="View User">
												<i class="fa fa-eye" ></i>
											</button>
											<c:if test="${user.isActive eq true}">
											<button class="btn btn-danger btn-xs" onclick="lockUser('${user.userId}')" title="Lock User">
												<i class="fa fa-lock" aria-hidden="true" ></i>
											</button>
											</c:if>
											<c:if test="${user.isActive eq false}">
											<button class="btn btn-success btn-xs" onclick="unlockUser('${user.userId}')" title="Unlock User">
