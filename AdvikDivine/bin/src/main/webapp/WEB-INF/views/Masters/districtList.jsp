	...
	<div class="panel-header bg-primary-gradient">
		<div class="page-inner">
			<div
				...
				<div>
					<h2 class="text-white mt-2 fw-bold">View District</h2>
				</div>
				<div class="ml-md-auto py-2 py-md-0">
					<a href="${contextPath}/"
						class="btn btn-white btn-border btn-round mr-2"><i
						class="fa fa-home"></i></a> <a href="#"
						class="btn btn-white btn-border btn-round"> View District</a>
				</div>
			</div>
		</div>
	</div>
								...
								<tbody id="tbd">
									<c:forEach items="${districtList}" var="district"
										...
										<tr>
											...
											<c:if test="${district.isActive eq true }">
												<td>Active</td>
											</c:if>
											<c:if test="${district.isActive eq false }">
												<td>InActive</td>
											</c:if>
											<td>
												<button class="btn btn-warning btn-sm" data-toggle="tooltip"
													title="Edit"
													onclick="editDistrictById('${district.districtId}')">
...
<script>
	...
	function editDistrictById(id){
	    $("#formId").attr('action','${contextPath}/admin/district/edit/'+ id +'');
	    $("#formId").submit();
   }
