...
$(document).ready(function(){
	...
	if(!minimize_sidebar){
		...
		if($('.wrapper').hasClass('sidebar_minimize')){
			...
			minibutton.html('<i class="fas fa-angle-double-right"></i>');
		}
		...
		minibutton.on('click', function() {
			if (mini_sidebar == 1) {
				...
				minibutton.html('<i class="fas fa-angle-double-left"></i>');
				mini_sidebar = 0;
			} else {
	...
	if(!toggle_overlay_sidebar){
		...
		if($('.wrapper').hasClass('is-show')){
			...
			overlaybutton.html('<i class="fas fa-angle-double-right"></i>');
		}
		...
		overlaybutton.on('click', function() {
			if (overlay_sidebar_open == 1) {
				...
				overlaybutton.html('<i class="fas fa-angle-double-left"></i>');
				overlay_sidebar_open = 0;
			} else {
...
$(function() {
		$('.card')
			.on( 'click', '.panel-actions a.fa-caret-up', function( e ) {
				...
				$card.find('.card-body, .card-footer').slideDown( 200 );
			})
			.on( 'click', '.panel-actions a.fa-caret-down', function( e ) {
				...
				$card.find('.card-body, .card-footer').slideUp( 200 );
			})
			.on( 'click', '.panel-actions a.fa-times', function( e ) {
				...
				if ( !!( $card.parent('div').attr('class') || '' ).match( /col-(xs|sm|md|lg)/g ) && $panel.siblings().length === 0 ) {
					$row = $card.closest('.row');
					$card.parent('div').remove();
